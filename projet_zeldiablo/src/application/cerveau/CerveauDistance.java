...
/**
 * Represente une IA qui doit rester a bonne distance du personnage
 */
public class CerveauDistance implements Cerveau {

	private int distanceVoulue;

	/**
	 * Constructeur de CerveauDistance.java pour une distance donnee
	 * 
	 * @param distanceVoulue Distance que l'IA va chercher a avoir entre elle et le
	 *                       joueur
	 */
	public CerveauDistance(int distanceVoulue) {
	...
	public String decider(int xm, int ym, int xp, int yp) {
		...
		if (distanceActuelle < this.distanceVoulue + 10 && distanceActuelle < this.distanceVoulue) {
			int deltaX = xm - xp;
			int deltaY = ym - yp;
			...
			if (degres >= -10 && degres <= 10)
				...
				return "NO";
		} else if (distanceActuelle > this.distanceVoulue + 10) {
			int deltaX = xm - xp;
			int deltaY = ym - yp;
